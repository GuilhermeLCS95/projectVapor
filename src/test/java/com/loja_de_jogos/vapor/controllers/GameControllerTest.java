...
class GameControllerTest {
  ...
  void createShouldReturnCreatedGame() throws Exception {
    ...
    ArgumentCaptor<GameCreationRequestDTO> captor =
        ArgumentCaptor.forClass(GameCreationRequestDTO.class);
  ...
  void getByIdShouldReturnNotFoundWhenGameDoesNotExist() throws Exception {
    ...
    mockMvc.perform(get("/games/{id}", 99L)).andExpect(status().isNotFound());
  ...
  void updateShouldReturnUpdatedGame() throws Exception {
    ...
    when(gameService.updateGame(any(Long.class), any(GameUpdateRequestDTO.class)))
        .thenReturn(response);
    ...
    ArgumentCaptor<GameUpdateRequestDTO> captor =
        ArgumentCaptor.forClass(GameUpdateRequestDTO.class);
  ...
  void deleteShouldCallService() throws Exception {
    mockMvc.perform(delete("/games/{id}", 1L)).andExpect(status().isOk());
