...
public record GameCreationRequestDTO(
    ...
    @NotNull(message = "Price is required.") @PositiveOrZero(message = "Price cannot be negative.")
        Double price,
    @ElementCollection @Enumerated(EnumType.STRING) @NotEmpty(message = "Genre cannot be empty.")
        List<Genre> genre,
    @NotNull(message = "Release date cannot be null.") Timestamp releaseDate,
    @PositiveOrZero Double userRating,
    @NotBlank(message = "Developer date cannot be null.") String developer,
    @NotBlank(message = "Publisher date cannot be null.") String publisher,
    Boolean hasDiscount,
    @ElementCollection @Enumerated(EnumType.STRING) @NotNull(message = "Age rating cannot be null.")
        AgeRating ageRating) {}
